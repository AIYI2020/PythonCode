...
try:
    conn = pymysql.connect(host="127.0.0.1", port=3306, user="root", password="a6225803", db="dbtest", )
    # 2.获取游标对象
    cur = conn.cursor()
    print("连接服务器成功!")
    print('')
    ...
    for file in files:
        if file.split('.')[-1] in ['csv']:
            ...
            f = pd.read_csv(file, encoding='gbk', dtype=object)  # 注意：如果报错就改成 encoding='utf-8' 试试
            # print(f)
            ...
            for item in columns:
                if 'int' == types[item]:
                    char = item + ' INT'
                elif 'float' == types[item]:
                    char = item + ' FLOAT'
                elif 'object' == types[item]:
                    char = item + ' VARCHAR(255)'
                elif 'datetime' == types[item]:
                    char = item + ' DATETIME'
                else:
                    char = item + ' VARCHAR(255)'
                table.append(char)
